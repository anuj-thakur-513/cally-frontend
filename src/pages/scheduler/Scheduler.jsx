...
import { Clock, Calendar, Send, Loader } from "lucide-react";
...
const Scheduler = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        let loadingtoast;
        if (submittingFlag) {
            loadingtoast = toast.loading("Try after few seconds , currently creating a meeting for you")
            setTimeout(()=>toast.dismiss(loadingtoast),1000)
        }
        setsubmittingFlag(true);
        if (!submittingFlag) {
            loadingtoast = toast.loading("Creating the meeting...")
                ...
                <form onSubmit={handleSubmit}>
                    <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="lg:w-2/5 space-y-8">
                            ...
                            <button
                                type="submit"
                                className={`w-full py-3 px-4 rounded-lg font-medium flex items-center justify-center space-x-2 transition duration-200 ${!date || !time || !title || !description
                                    ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                                    : "bg-blue-500 text-white hover:bg-blue-600"
                                    }`}
                                disabled={!date || !time || !title || !description || submittingFlag}
                            >
                                <Send className={`${!submittingFlag ? "block" : "hidden"} w-5 h-5 `}   />
                                <span className={`${!submittingFlag ? "block" : "hidden"} `} >Send Meeting Request</span>
                                <Loader className={`${submittingFlag ? "block" : "hidden"} animate-spin z-10`} />
                            </button>
                        </div>
                                ...
                                <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-2">
                                    {TIME_OPTIONS.map((slot) => (
                                        <button
                                            ...
                                            className={`py-2 px-3 cursor-pointer rounded-lg text-sm font-medium transition duration-200 ${time === slot
